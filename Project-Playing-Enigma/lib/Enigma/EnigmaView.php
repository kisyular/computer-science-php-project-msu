...
class EnigmaView extends BaseView
{
        public function presentForm(){
            for($j=0;$j<3;$j++){
                $letter = $this->getSystem()->getMyenigma()->getRotorSetting($j+1);
                $class = "key-A".($j+1);
            ...
            for($i=0;$i < 26;$i++){
                ...
                $class4 = "light";
                $key = chr(65+$i);
                if($key === $this->getSystem()->getPressedKey()){
                ...
                if($key === $this->getSystem()->getLightKey()){
...
<div class="welcome">
HTML;
            $html .= "<h1>Greetings, ".$this->getSystem()->getUsername().", and Welcome to the Endless Enigma!</h1></div>";
            return $html;
        }
}